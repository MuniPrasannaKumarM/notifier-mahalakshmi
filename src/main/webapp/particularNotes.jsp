	...
	<div class="container-fluid">
		...
		<ul class="nav navbar-nav navbar-left">
			...
			<div id="mySidenav" class="sidenav">
				...
				<h3 style="color: white;">Hai ${userName}</h3>
				<a id="noteBookView" href="showAllNoteBooks">NoteBooks</a> 
				<a id="listOfNotes" href="listAllNotes">Notes</a> 
			...
			<a href="#" class="dropdown-toggle glyphicon glyphicon-bell notify"
				...
				<ul class="dropdown-menu notify-drop">
					...
					<div class="drop-content">
						<c:forEach var="remainder" items="${noteObjDate}">
                            <li class="col-md-offset-1"><div >
                                <h5 class="" style="color: brown;">${remainder.noteName}</h5>
                                <h6>Start Date : ${remainder.startDate}</h6>
                                <h6>End Date : ${remainder.endDate }</h6>   
		...
		<ul class="nav navbar-nav navbar-right">
			...
			<li><a  id="addNewNote" href="" data-toggle="modal" data-id="${noteBook.id}"
				data-target="#myModal" style="visibility: ${ui}" class="open-AddBookDialog">NEW NOTE</a></li>
		</ul>
	</div>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
					...
					<form action="createNote" method="post" onsubmit="return validateNoteForm()">
                        <label for="notebook_id" class="col-md-3 col-form-label">NoteBook ID : </label>
						<input type="text" name="notebook_id" class="form-control"
									id="notebook_id" placeholder="Notebook id" value="">
...
<div class="column right">
	 <div class="row">
		...
		<div class="col-md-8 text-left changes">
			<h1>Notes in <c:out value="${noteBook.noteBookName}" /></h1>
		</div>
	</div><br>
	<%int i=0;%>
								...
								<div class="panel-body">
									<div class="row">
										<div class="col-md-1 text-left">
											<a  id="notedetails" href="showNoteDetails?itemId=${item.id}"><c:out value="${item.noteName}" />
												
												</a>
										</div>
										<div class="col-md-2 text-left">Started on: <c:out value="${item.startDate}" /></div>
										
										<c:choose>
											<c:when
												test="${item.statusName == 'complete'}">
												<div>
													<button type="button" id="status" class="btn btn-success btn-md medium">Complete</button>
												</div>
											</c:when>
											<c:when
												test="${item.statusName == 'inProgress'}">
												<div>
										...
										<div class="col-md-10 text-right">
											<!--<a id="editNote"  href="editNote?itemId=${item.id}">--><a  id="addNewNote" href="" data-toggle="modal" 
											
											data-id="${item.id}" data-notebookid = "${noteBook.id}" data-name="${item.noteName}" data-startdate = "${item.startDate}"
											data-enddate="${item.endDate}" data-remainderdate="${item.remainderDate}" 
											data-statusname = "${item.statusName}" data-tagname = "${item.tagName}"
											data-notedescription = "${item.noteDescription }"
				data-target="#editModal" style="visibility: ${ui}" class="Maha"><span
												class="glyphicon glyphicon-edit" >Edit</span></a>
										</div>
										<div>
											<a href="deleteNote?itemId=${item.id}&notebook_id=${noteBook.id}"><span
	...
	<div class="modal fade" id="editModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
					...
					<form action="editNote" method="post" onsubmit="return validateNoteForm()">
					 <label for="notebook_id" >NoteBook ID : </label><br>
                        <input type="text" name="notebook_id" class="form-control"
									id="notebook_id" placeholder="Note Book id" value="">
						<label for="notebook_id" >Note ID : </label>
						<input type="text" name="note_id" class="form-control"
									id="note_id" placeholder="Note id" value="">
    ...
    $(document).on("click", ".open-AddBookDialog", function() {
		var notebook_id = $(this).data('id');
		$(".modal-body #notebook_id").val(notebook_id);
		$('#myModal').modal('show');
	});
    $(document).on("click", ".Maha", function() {
        var notebook_id = $(this).data('notebookid');
		var note_id = $(this).data('id');
		var noteName = $(this).data('name');
		var startDate = $(this).data('startdate');
		var endDate = $(this).data('enddate');
		var remainderDate = $(this).data('remainderdate');
		var statusName = $(this).data('statusname');
		var tagName = $(this).data('tagname');
		var noteDescription = $(this).data('notedescription');
		$(".modal-body #notebook_id").val(notebook_id);
		...
		$(".modal-body #remainderDate").val(remainderDate);
		$(".modal-body #statusName").val(statusName);
		$(".modal-body #tagName").val(tagName);
		$(".modal-body #noteDescription").val(noteDescription);
